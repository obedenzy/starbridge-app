-- Update the handle_new_user function to properly populate profiles table with full_name and business_id
...
DECLARE
  ...
  _hint TEXT;
  _context TEXT;
  _generated_business_id BIGINT; -- Change to BIGINT to match business_settings table
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = NEW.id AND role = 'super_admin') THEN
    ...
    -- Assign business_admin role to the user who just created/updated their business settings
    BEGIN
...
-- Update existing profiles that may be missing business_id
