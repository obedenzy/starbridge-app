-- First, create the missing user_roles entry for the current user
INSERT INTO public.user_roles (user_id, role)
VALUES ('c4ca74ce-afc5-4349-a99e-a3774e6ecc2d', 'business_admin')
ON CONFLICT (user_id, role) DO NOTHING;

-- Create the missing business_users entry for the current user
...
-- Now fix the handle_new_user function to work properly going forward
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = NEW.id AND role = 'super_admin') THEN
    RAISE NOTICE 'handle_new_user: User % is NOT a super_admin. Proceeding with business_settings creation and business_admin role assignment.', NEW.id;
    ...
    END;
  ELSE
    RAISE NOTICE 'handle_new_user: User % is a super_admin. Skipping business_settings creation and business_admin role assignment.', NEW.id;
  END IF;
